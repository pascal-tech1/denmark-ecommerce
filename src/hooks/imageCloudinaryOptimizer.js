...
export const uptimizeCloudinaryImage = async (optimizationStr, originalImageurl) => {
  const cloudinaryBaseUrl = "https://res.cloudinary.com/da3q9dbku/image/upload/";

  try {
    // Fetch image metadata
    const metadataUrl = originalImageurl.replace('/upload/', '/upload/fl_getinfo/');
...
console.log(imageWidth)
    ...
    const baseUrlEndIndex = originalImageurl.indexOf(cloudinaryBaseUrl) + cloudinaryBaseUrl.length;
